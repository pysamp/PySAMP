...
def on_ready():
    ...
    disable_interior_enter_exits()
    set_weather(2)
    #limit_global_chat_radius(300.0)
...
@Player.on_connect
def on_player_connect(player: Player.from_registry):
...
@Player.on_text
def on_player_text(player: Player.from_registry, text: str):
    ...

@Player.on_request_class
def on_player_request_class(player: Player.from_registry, classid: int):
    if player.is_npc():
        return
...
@Player.on_update
def on_player_update(player: Player.from_registry):
...
callbacks.hook() # Hooks all callbacks from user's "python" module.